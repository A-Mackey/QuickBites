  ...
  methods: {
    ...
    parseArray(data) {
      ...
      for (var i=0; i<n;i++){
        ...
        if (tmp[i].Includes.toLowerCase().search(this.query.toLowerCase()) == -1 && this.Includes!="") {
          include=0;
          console.log("slot1");
        }
        if (tmp[i].MaxBudget>Number(this.price) && this.price!=""){
          include=0;
          console.log("slot2");
        }
        if (tmp[i].MinPeople<Number(this.people) && this.people!=""){
          include=0;
          console.log("slot3");
        }
        if (tmp[i].MaxTime>Number(this.time) && this.time!=""){
          include=0;
          console.log("slot4");
        }
        if(include==0){
